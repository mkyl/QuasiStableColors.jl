...
function _lifted_opt(A, b::Vector, c::Vector; obj=MathOptInterface.MIN_SENSE, args...)
    ...
    # n, m = size(Ã)
    # G, W, U, V = matrix_to_graph(Ã)
    # @debug "converted matrix to graph"

    # # last row and column have their own color each
    # c_row = pop!(U, n)
    # b_col = pop!(V, n + m)

    # # find the coloring
    # P_base = [collect(U), collect(V), [c_row], [b_col]]
    # @time P = refine_stable(G; weights=W, warm_start=P_base, args...)

    # P_row = filter((X) -> !isempty(X ∩ U), P)
    # P_col = filter((X) -> !isempty(X ∩ V), P)

    # @assert length(P_row) + length(P_col) == length(P) - 2

    # P_col_reindex = [x .- n for x in P_col]

    # convert coloring back to matrix
    # U, V = P_to_UV(P_row), P_to_UV(P_col_reindex)
